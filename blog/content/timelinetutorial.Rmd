...
## Adjustments
Open the html file just created and look for `storyjs-embed.js`. The file location will show 
```{r, eval=FALSE}
/Library/Frameworks/R.framework/Versions/3.0/Resources/library/rCharts/libraries/timeline/js/storyjs-embed.js 
```
Change it to `timeline/js/storyjs-embed.js` but don't forget to copy the full timeline folder to your current working directory. If you don't do this step the timeline won't display in a computer without R installed.
Now, if you open your timeline the dates will be a little bit ugly, look for `startDate` and the dates will be shown as "1/1/67 00:00", you can change it to "1967" without further steps but do the same to any `startDate`.
